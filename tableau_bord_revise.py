...
for nom_actif in resultats_df.index:
    ...
    resultats_df.loc[nom_actif, 'rendement_net_annualise'] = (
        ((resultats_df.loc[nom_actif, 'gain_net_moyen'] + sim.montant_pret) / sim.montant_pret) ** (1/10) - 1
    ) * 100
...
# Titre et paramètres
plt.suptitle("TABLEAU DE BORD - ANALYSE DES OPTIONS D'INVESTISSEMENT", fontsize=24, y=0.995)
texte_param = (
...
explanation_text = (
    ...
    "• Rendement Net Annualisé : Taux de rendement annuel équivalent basé sur le gain net\n\n"
    "• Ratio Gain/Coût : Combien d'euros de gain net pour chaque euro d'intérêt payé\n\n"
    "IMPORTANT : Cette simulation considère que vous empruntez 200 000 € à la banque, investissez ce montant, et remboursez\n"
    "le prêt selon l'échéancier convenu. Le gain net représente ce que vous gardez après avoir remboursé le capital et les intérêts.\n\n"
    "REMARQUE SUR LES RÉSULTATS :\n"
    "Les rendements négatifs indiquent que l'investissement ne génère pas suffisamment pour couvrir les intérêts du prêt.\n"
    "Les rendements positifs indiquent que l'investissement génère plus que les intérêts, créant un bénéfice net.\n"
...
plt.savefig(f"{repertoire_sortie}/tableau_bord_revise.png", dpi=300, bbox_inches='tight')
plt.savefig(f"{repertoire_sortie}/tableau_bord_revise.pdf", bbox_inches='tight')
print(f"\nTableau de bord révisé enregistré dans {repertoire_sortie}/tableau_bord_revise.png et .pdf")

# Afficher le chemin vers le fichier généré
print(f"\nVous pouvez consulter le tableau de bord révisé ici: {os.path.abspath(f'{repertoire_sortie}/tableau_bord_revise.png')}")
